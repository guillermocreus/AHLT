...
def check_interaction(tree, entities, e1, e2) :
   ...
   # Check pattern: LCS is a verb and it has the modifier "should" --> advise
   ...
   # Check pattern: A word in between both entities belongs to a certain list
   p = patterns.check_wib(tree,tkE1,tkE2,entities,e1,e2)
   if p is not None: return p

   # p = patterns.check_pattern_G3(tree,tkE1,tkE2)
   # if p is not None: return p
