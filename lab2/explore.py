...
# -----------------
# check pattern:  LCS is a verb, one entity is under its "nsubj" and the other under its "obj"
def check_pattern_G(tree, entities, e1, e2):
    ...
    if tkE1 is not None and tkE2 is not None:
        ...
        if tree.get_tag(lcs)[0:2] == "VB":
            ...
            for c in tree.get_children(lcs):
                if tree.get_lemma(c) == 'should':
                    return [tree.get_lemma(lcs)+'_'+tree.get_tag(lcs)[0:2]]

    return None

def check_pattern_G2(tree, entities, e1, e2):
    ...
    if tkE1 is not None and tkE2 is not None:
        ...
        return [tree.get_lemma(lcs)+'_'+tree.get_tag(lcs)]

    return None


def check_pattern_G3(tree, entities, e1, e2):
...
def check_pattern_G4(tree, entities, e1, e2):
    ...
    if tkE1 is not None and tkE2 is not None:
        ...
        for c in tree.get_children(lcs):
            if tree.get_rel(c) == 'obj':
                if not dont_return:
                    return [tree.get_lemma(lcs)+'_'+tree.get_lemma(c)]
...
for f in listdir(datadir):
    ...
    for s in sentences:
        ...
        for p in pairs:
            ...
            match = check_pattern_G4(analysis, entities, id_e1, id_e2)
            ####################################################
            # match = []
            ...
            elif match is None and typ == 'effect':
                print()
                print('ESCAPE')
                print('-------------')
                print('sentence:', stext)
    ...
    for m in countP:
        for t in countMT:
            ...
            if k in countPT:
                ...
                key_G = t + '|' + m
                if t != 'null':
                    dict_G.append((countPT[k]/countP[m], countPT[k], countP[m], key_G))

dict_G.sort(reverse=True)

print('\n'*5)
for (a, b, c, d) in dict_G:
    print(d, ':', a, ' -- ', b, '/', c)
